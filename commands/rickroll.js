exports.run = async function (Memer, msg) {
	...
	if (!Memer.client.voiceConnections.get(msg.channel.guild.id)) {
		...
		conn.on('end', async() => {
			...
			if (Memer.client.voiceConnections.get(msg.channel.guild.id)) {
				console.error(`${Date} hey rickroll.js fucked up`)
		...
		await msg.addReaction('❌')
		Memer.reply('There\'s only *one* Rick Astley, dude. Please wait until the current sound is done or the ear-rape ghost will visit you in your sleep!', msg)
	}

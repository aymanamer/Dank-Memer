...
exports.run = async function (Memer, msg) {
	...
	if (!Memer.client.voiceConnections.get(msg.channel.guild.id)) {
		...
		conn.once('end', async() => {
			...
			if (Memer.client.voiceConnections.get(msg.channel.guild.id)) {
				console.error(`${Date} hey mlg.js fucked up`)
		...
		await msg.addReaction('❌')
		Memer.reply('I can only have one sound at once, dude. Please wait until the current sound is done or the ear-rape ghost will visit you in your sleep!', msg)
	}

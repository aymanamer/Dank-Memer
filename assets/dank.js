...
exports.run = (URL) => {
	return new Promise(async (resolve, reject) => {
		...
		const triggered = await Jimp.read(path.join(__dirname, '..', 'assets/horn.png'))
		triggered.resize(100, 100)
		triggered.rotate(315)
		const triggered2 = triggered.clone()
		triggered2.resize(130, 130)
		triggered2.rotate(350)
		triggered2.flip(true, false)
		const overlay = await Jimp.read(path.join(__dirname, '..', 'assets/red.png'))
		overlay.opacity(0.2)
		const hit = await Jimp.read(path.join(__dirname, '..', 'assets/hit.png'))
		hit.resize(40, 40)
		const gun = await Jimp.read(path.join(__dirname, '..', 'assets/gun.png'))
		gun.resize(250, Jimp.AUTO)
		const faze = await Jimp.read(path.join(__dirname, '..', 'assets/faze.png'))

...
const ipcHandler = require('./handlers/ipcHandler.js')
...
class Memer extends Base {
  ...
  registerIpcBroadcast () {
    process.on('message', async (message) => {
      ...
      if (message._eventName !== 'global' || !event) {
        return
      }

      try {
        await ipcHandler.processIncomingMessage(event, data)
      } catch (exception) {
        this.log(exception.stack, 'error')
      }
    })
  }

  launch () {
    this.registerIpcBroadcast()
    this.loadCommands()
